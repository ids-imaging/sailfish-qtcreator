...
void MerSdk::setVdiInfo(const QString &vdiPath, int vdiCapacityMb, int vdiSizeOnDiskMb)
{
    m_vdiPath = vdiPath;
...
void MerSdk::setVdiSizeMb(int sizeMb)
{
    m_vdiCapacityMb = sizeMb;
}

QString MerSdk::vdiPath() const
{
    return m_vdiPath;
}

int MerSdk::vdiSizeOnDiskMb() const
{
...
QVariantMap MerSdk::toMap() const
{
    ...
    result.insert(QLatin1String(VDI_PATH), vdiPath());
    result.insert(QLatin1String(VDI_CAPACITY_MB), vdiCapacityMb());
    result.insert(QLatin1String(VDI_SIZE_MB), vdiSizeOnDiskMb());
    int count = 0;
    foreach (const MerTarget &target, m_targets) {
        if (!target.isValid())
...
bool MerSdk::fromMap(const QVariantMap &data)
{
    ...
    setVdiInfo(data.value(QLatin1String(VDI_PATH)).toString(),
               data.value(QLatin1String(VDI_CAPACITY_MB)).toInt(),
               data.value(QLatin1String(VDI_SIZE_MB)).toInt());
