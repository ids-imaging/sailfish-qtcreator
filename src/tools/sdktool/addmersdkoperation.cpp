...
QString AddMerSdkOperation::argumentsHelpText() const
{
    ...
    return indent + QLatin1String(MER_PARAM_INSTALLDIR) + QLatin1String(" <DIR>            mer sdk install directory (required).\n")
         ...
         + indent + QLatin1String(MER_PARAM_MEMORY_SIZE) + QLatin1String(" <NUMBER> MB          mersdk virtual machine memory size (required).\n")
         + indent + QLatin1String(MER_PARAM_CPU_COUNT) + QLatin1String(" <NUMBER>          mersdk virtual machine cpu count (required).\n")
         + indent + QLatin1String(MER_PARAM_VDI_CAPACITY) + QLatin1String(" <NUMBER> MB    mersdk virtual machine vdi size (required).\n")
...
int AddMerSdkOperation::execute() const
{
    ...
    const QVariantMap result = addSdk(map, m_name, m_autoDetected, m_sharedHomePath, m_sharedTargetsPath, m_sharedSshPath,
                                      ...
                                      m_wwwPort, m_headless, m_memorySizeMb, m_cpuCount, m_vdiSizeMb);
...
void addVirtualMachineParameters(Operation::KeyValuePairList *data, const QString &sdk, const QString &virtualMachineName)
{
    if (!virtualMachineName.isEmpty()) {
        Mer::Internal::MerVirtualBoxManager virtualBoxManager;
        const auto info = virtualBoxManager.fetchVirtualMachineInfo(virtualMachineName, true);
        *data << Operation::KeyValuePair(QStringList() << sdk << QLatin1String(Mer::Constants::VDI_SIZE_MB), QVariant(info.vdiSizeOnDisk));
        *data << Operation::KeyValuePair(QStringList() << sdk << QLatin1String(Mer::Constants::VDI_PATH), QVariant(info.vdiPath));
    }
}
                                          ...
                                          int vdiSizeMb)
{
    ...
    data << KeyValuePair(QStringList() << sdk << QLatin1String(Mer::Constants::VDI_CAPACITY_MB), QVariant(vdiSizeMb));
    addVirtualMachineParameters(&data, sdk, sdkName);
